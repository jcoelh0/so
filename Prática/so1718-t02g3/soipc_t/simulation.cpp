...
static void go()
{
   ...
   pthread_t thr[global->NUM_STUDENTS + 2];  
   pthread_create(thr, NULL, mainLogger, NULL);
   pthread_create(thr+1, NULL, mainLibrarian, NULL);
   for(int i=2; i<global->NUM_STUDENTS+2; i++)
   {
	 _Student_* temp = students[i];
     pthread_create((thr+2)+i, NULL, mainStudent, temp);
   }
	
   for (int i=0; i<global->NUM_STUDENTS + 2; i++)
   {
